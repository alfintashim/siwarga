...
@section('title')
<title>Pengurus RT | Informasi</title>
@endsection
...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active"><i class="fa fa-th"></i> Informasi</li>
  </ol>
</section>
          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tr>
              ...
              <td align="center" width="30">
                ...
                @else
                <img width="40px" class="img-circle" src="{{ asset('uploads/berita/'.$data->gambar) }}"/>   
                @endif
              </td>
              <td>
                <a href='{{ URL::to('/pengurus/berita/'.$data->id) }}'>{{ $data->judul }}</a>
              </td>
              <td align='center'>{{ date('j M Y', strtotime($data->created_at)) }}</td>
              <td align='center' width='140'>
                <div class="btn-group">
                  <a href='{{ URL::to('pengurus/berita/'.$data->id) }}'>
                    <button type="button" class="btn btn-info"><i class="fa fa-info-circle"></i></button>
                  </a>
                  <a href='{{ URL::to('pengurus/berita/'.$data->id.'/edit') }}'>
                    <button type="button" class="btn btn-warning"><i class="fa fa-edit"></i></button>
                  </a>
                  <a>
                    <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#modal-danger"><i class="fa fa-trash"></i></button>
                  </a>
                </div>
                <form method="POST" action="{{ URL::to('/pengurus/berita/'.$data->id.'/delete')}}" enctype="multipart/form-data">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          ...
                          <p>Apakah anda yakin akan menghapus informasi berjudul</p>
                          <p> <b>{{ $data->judul }}</b> ?</p>
                          <br>
